...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h1 class="h3 fw-bold text-dark mb-1">Store Management</h1>
        <p class="text-muted small">View and manage all registered merchant stores.</p>
    </div>
    <a href="{{ route('super_admin.create_tenant') }}" class="btn btn-dark btn-sm shadow-sm">
    ...
    <div class="card-header bg-white py-3">
        <div class="row g-3 align-items-center justify-content-between">
            <div class="col-md-auto">
                <h5 class="mb-0 text-dark fw-bold">All Stores</h5>
            </div>
            <div class="col-md-auto">
                <form action="{{ route('super_admin.tenants') }}" method="GET" class="d-flex gap-2">
                    <!-- Status Filter -->
                    <div class="btn-group" role="group">
                        <a href="{{ route('super_admin.tenants', array_merge(request()->query(), ['status' => null])) }}" class="btn btn-outline-secondary btn-sm {{ !request('status') ? 'active' : '' }}">All</a>
                        <a href="{{ route('super_admin.tenants', array_merge(request()->query(), ['status' => 'active'])) }}" class="btn btn-outline-success btn-sm {{ request('status') == 'active' ? 'active' : '' }}">Active</a>
                        <a href="{{ route('super_admin.tenants', array_merge(request()->query(), ['status' => 'inactive'])) }}" class="btn btn-outline-danger btn-sm {{ request('status') == 'inactive' ? 'active' : '' }}">Inactive</a>
                    </div>
